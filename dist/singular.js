var singular = (() => {
  ...
  var singular_exports = {};
  __export(singular_exports, {
    addScript: () => addScript,
    addStyle: () => addStyle,
    changed: () => changed,
    configure: () => configure,
  ...
  function route(requestUrl, outletSelectors) {
    try {
  ...
  function changed(changedElements = [document.body]) {
  ...
  function setPathClass({ prefix, baseHref }) {
    if (!prefix) {
      prefix = "path";
    }
    if (!baseHref) {
  ...
  return __toCommonJS(singular_exports);
})();
